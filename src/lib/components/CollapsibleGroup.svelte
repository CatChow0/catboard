<script lang="ts">
	import type { CollapsibleGroupItem, Service, DashboardItem as DashboardItemType } from '$lib/stores/dashboard';
	...
	async function handleSaveEdit() {
		...
		updates.config = { ...item.config, outlineColor: editOutlineColor || undefined };
...
<div class="group collapsible" class:collapsed={collapsed} style={outlineStyle} data-group-id={item.id}>
	<div class="group-header" onclick={() => { if (!showEditModal) collapsed = !collapsed; }} role="button" tabindex="0" onkeydown={(e) => e.key === 'Enter' && !showEditModal && (collapsed = !collapsed)}>
		<svg class="chevron" class:rotated={!collapsed} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
			<path d="M9 18l6-6-6-6" />
		</svg>
		<span class="group-title">{item.title}</span>
	</div>
	{#if $isEditing}
		<button class="btn-delete" onclick={(e) => { e.stopPropagation(); onremove?.(item.id); }} title="Remove group">
			<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
				<path d="M18 6L6 18M6 6l12 12" />
			</svg>
		</button>
		<button class="btn-edit" onclick={(e) => { e.stopPropagation(); openEditModal(); }} title="Edit group">
	...
	{#if showEditModal}
		...
		<div class="edit-modal">
			...
			<div class="field">
				<label>Title</label>
				<input type="text" bind:value={editTitle} placeholder="Group name" onkeydown={(e) => e.key === 'Enter' && handleSaveEdit()} />
			</div>
			<div class="field">
